...
DPTable::DPTable(ReadSet* read_set, vector<unsigned int> read_marks, vector<unsigned int> recombcost, vector<array<vector<unsigned int> , 3>> genotypes, std::vector<std::vector<int>> triples, unsigned int number_of_ind)
: read_set(read_set), read_marks(std::move(read_marks)), recombcost(std::move(recombcost)), genotypes(std::move(genotypes)),triples(std::move(triples)), number_of_ind(std::move(number_of_ind)), optimal_score(0u), optimal_score_index(0u),
...
void DPTable::compute_table() {
  unsigned int num_recombs = std::pow(4, num_of_trios);
...
// }
  ...
  while(next_indexer != 0) {
   // std::cout<<"column"<<n<<endl;
    ...
    vector<ColumnCostComputer> cost_computers;
	...
	for(auto& cost_computer : cost_computers) {
      ...
      std::vector<std::vector<unsigned int>> tempgeno;
      for (auto& genotriple: genotypes)
      {
	  tempgeno.emplace_back({genotriple[0][n],genotriple[1][n],genotriple[2][n]});
      }
      for(auto& cost_computer : cost_computers) {
...
void DPTable::get_super_reads(std::vector<ReadSet*>* output_read_set, vector<unsigned int>* transmission_vector) {
 ...
 // assert(output_read_setc != 0u);
  ...
  for(unsigned int i=0;i<number_of_ind;i++)
  {
  ...
  if (index_backtrace_table.empty()) {
    ...
    while (column_iterator.has_next()) {
      ...
      ColumnCostComputer cost_computer(*column, read_marks, v.inheritance_value, triples);
      cost_computer.set_partitioning(v.index);
      std::vector<std::vector<unsigned int>> tempgeno;
      for (auto& genotriple: genotypes)
      {
	  tempgeno.emplace_back({genotriple[0][i],genotriple[1][i],genotriple[2][i]});
      }
      ...
      // TODO: compute proper weights based on likelihoods.
       ...
       {
	 superreads[k].first->addVariant(positions->at(i), population_alleles[id_of_individuals[k]].ind.first, 0);
	 superreads[k].second->addVariant(positions->at(i), population_alleles[id_of_individuals[k]].ind.second, 0);
       }
  ...
  for(unsigned int k=0;k<id_of_individuals.size();k++){
    (*output_read_set)[k]->add(superreads[k].first);
    (*output_read_set)[k]->add(superreads[k].second);
  }
}
