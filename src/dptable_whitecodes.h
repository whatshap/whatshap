...
private:
  ...
  std::vector<bool> indx;
  
 
 // void compute_table();
  void compute_table(std::vector<bool> indx);
...
public:
  ...
  //DPTable(ReadSet* read_set, bool all_heterozygous = false);
  DPTable_whitecodes(ReadSet* read_set,std::vector<bool> indx, bool all_heterozygous = false);
