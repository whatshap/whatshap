...
PedigreePartitions::PedigreePartitions(const Pedigree& pedigree, unsigned int transmission_vector, unsigned int ploidy) 
	: pedigree(pedigree),
	...
	if (ploidy == 2) {
		...
		// for polyploids, ignore any trio relationships
		partition_count = ploidy * pedigree.size(); 
		int p = 0;
		for(size_t i=0; i<pedigree.size(); ++i){
...
// assumes diploid case, for polyploid genomes, currently no trio information is used.
