...
public:
	...
	std::vector<unsigned int> getVariantQuality(size_t variant_idx) const;
	void setVariantQuality(size_t variant_idx, std::vector<unsigned int> quality);
...
private:
	typedef struct enriched_entry_t {
		Entry entry;
		int position;
		enriched_entry_t(int position, int allele, std::vector<unsigned int> quality) :
			entry(0,allele,quality), position(position) {}
	} enriched_entry_t;
