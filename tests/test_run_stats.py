...
def test_stats1():
	with TemporaryDirectory() as tempdir:
		...
		run_stats(vcf='tests/data/phased1.vcf', tsv=outtsv, sample='sample1', chr_lengths='tests/data/chr-lengths.txt')
...
def test_stats2():
	with TemporaryDirectory() as tempdir:
		outtsv = tempdir + '/output.tsv'
		run_stats(vcf='tests/data/phased3.vcf', tsv=outtsv, sample='sample1', chr_lengths='tests/data/chr-lengths.txt')
		lines = [l.split('\t') for l in open(outtsv)]
		assert len(lines) == 4
		Fields = namedtuple('Fields', [f.strip('#\n') for f in lines[0]])
		entry_chrA, entry_chrB, entry_all = [Fields(*l) for l in lines[1:]]
