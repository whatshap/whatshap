...
def test_selection():
	reads = string_to_readset("""
	  ...
	  1    1
	""")
	selected_reads, new_components, stats = readselection(reads, max_cov = 1, bridging = False)
	...
	selected_reads, new_components ,stats= readselection(reads, max_cov = 3, bridging = False)
	assert selected_reads == set([1,3,5,7]), str(selected_reads)
	selected_reads, new_components, stats = readselection(reads, max_cov = 3, bridging = True)
