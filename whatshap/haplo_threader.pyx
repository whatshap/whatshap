...
cdef class HaploThreader:
	...
	def computePathsBlockwise(self, vector[uint32_t]& blockStarts, vector[vector[uint32_t]]& covMap, vector[vector[double]]& coverage, vector[vector[uint32_t]]& consensus, vector[uint32_t]& genotypes, vector[vector[vector[double]]]& clusterDissim):
	...
	def computePaths(self, uint32_t start, uint32_t end, vector[vector[uint32_t]]& covMap, vector[vector[double]]& coverage, vector[vector[uint32_t]]& consensus, vector[uint32_t]& genotypes, vector[vector[vector[double]]]& clusterDissim):
		cdef vector[vector[uint32_t]] path
