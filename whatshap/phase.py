...
from .ilp import phase_by_flipping_ilp, phase_by_bigM_ilp, phase_by_chen_et_al_ilp
...
def run_whatshap(phase_input_files, variant_file, reference=None,
        ...
        mapping_quality=20, max_coverage=15, ilp=False, ilp_variant='flipping', distrust_genotypes=False, include_homozygous=False,
    ...
    max_coverage
    ilp -- use ILP to solve MEC score
    ilp_variant -- in case of ILP, which formulation to use
    distrust_genotypes
    include_homozygous
        ...
        for variant_table in vcf_reader:
            ...
            for representative_sample, family in families.items():
                ...
                with timers('phase'):
                    ...
                    if ilp:
                        logger.info('Method: %s', ilp_variant)
                        if ilp_variant == 'flipping':
                            superreads_list = phase_by_flipping_ilp(all_reads, not distrust_genotypes)
                            transmission_vector = []
                        if ilp_variant == 'chen_et_al':
                            superreads_list = phase_by_chen_et_al_ilp(all_reads, not distrust_genotypes)
                            transmission_vector = []
                        if ilp_variant == 'bigM':
                            superreads_list = phase_by_bigM_ilp(all_reads, not distrust_genotypes)
                            transmission_vector = []
                    else:
...
def add_arguments(parser):
    ...
    arg('--ilp_variant', choices=('flipping', 'chen_et_al', 'bigM'), default='flipping', help='ILP formulation (default: %(default)s)', dest='ilp_variant')
