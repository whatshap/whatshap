...
# Takes as input the chromosome wise ref-read kmer pair counts from whatshap call,
# calculates the eprobability for observing each ref-read kmer pair across the whole genome 
	...
	for file in glob_path:
		...
		for line in reader:
			...
			if new_key in probs:
				...
				probs[new_key]= count
	#storing the total read kmers mapping to a ref kmer in comb_count and unique read kmers mapping to each ref kmer in seen_comb			
...
if __name__ == "__main__":
	...
	parser.add_argument("-i", "--inputfile", type=str, help="The path to folder containing ref-read kmer pair counts obtained from whatshap call", required=True)
	parser.add_argument("-o", "--outputfile", type=str, help="A table of phred scores based on probilities calculated using kmer pair counts", required=True)
	parser.add_argument("-e", "--epsilon_value", type=str, help="The pseudocount value for kmer pairs that are not observed ", required=True)
