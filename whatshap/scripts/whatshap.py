...
from ..core import Read, ReadSet, DPTable, DPTable_whitecodes, IndexSet
...
def find_components(superreads_whitecodes, reads):
    """
    ...
    phased_positions = [ v1.position for v1, v2 in zip(*superreads_whitecodes)
    ...
    logger.info('No. of variants considered for phasing: %d', len(superreads_whitecodes[0]))
...
def run_whatshap(bam, vcf,
    ...
    with ExitStack() as stack:
        ...
        for sample, chromosome, variants in vcf_reader:
            ...
            partitioningread=dp_table.get_optimal_partitioning();
            
            logger.info(superreads)            
                      
            phased_positions = [ v1.position for v1, v2 in zip(*superreads)if (v1.allele, v2.allele) in ((0, 1), (1, 0))]
            
            position_to_index = { position: index for index, position in enumerate(phased_positions) }
            partition_0=list()
            partition_1=list() 
            numofreads=len(sliced_reads)
            
            partitioned_readset=BitArray(numofreads)        

            for index,read in enumerate(sliced_reads):
                cost0=0
                cost1=0
                for variant in read:
                    if variant.position in phased_positions:
                        if superreads[0][position_to_index[variant.position]].allele != variant.allele:
                            cost0=cost0+variant.quality
                        if superreads[1][position_to_index[variant.position]].allele != variant.allele:
                            cost1=cost1+variant.quality
                if cost0>=cost1:
                    partition_0.append(index)
                    partitioned_readset[index]=1
                else:
                    partition_1.append(index)
                    partitioned_readset[index]=0
            #random initialisation
            partitioned_readset_random=BitArray(numofreads)
            partitioned_readset_random=np.random.randint(2, size=numofreads)
            dp_table_whitecodes = DPTable_whitecodes(sliced_reads, partitioned_readset,all_heterozygous)
            # get the mec score
            mec_score_whitecodes = dp_table_whitecodes.get_optimal_cost()
            partitioning_whitecodes = dp_table_whitecodes.get_optimal_partitioning();
            # ... and do the backtrace to get the solution
            superreads_whitecodes = dp_table_whitecodes.get_super_reads()            
            logger.info(superreads_whitecodes)

                        
            print('MEC score of phasing: %d', mec_score)
            print('MEC score of phasing: %d', mec_score_whitecodes)
            #print('optimal partitioning')
            #print(partitioningread)
            #print('optimal partitioning whitecodes')
            #print(partitioning_whitecodes)
            n_homozygous = sum(1 for v1, v2 in zip(*superreads)
                if v1.allele == v2.allele and v1.allele in (0, 1))
            stats.n_homozygous += n_homozygous

            components = find_components(superreads_whitecodes, sliced_reads)
            ...
            vcf_writer.write(chromosome, sample, superreads_whitecodes, components)
